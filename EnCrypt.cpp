...
ConstMixCrypto{ 0x02, 0x03, 0x01, 0x01, 0x01, 0x02, 0x03,
...
void ENCRYPT::ByteSwappingCrypto(BYTE *Data)
{
    for (int i = 0; i < 16; i++)
    {
...
void ENCRYPT::ColumnMixingCrypto(BYTE *Data)
{
    ValueOwnCrypto = 0;
    ValueTwoCrypto = 0;
    for (int i = 0; i < 16; i++)
    {
        ...
        Data[i] = (PolinomByte(&ConstMixCrypto[ValueOwnCrypto * 4], &ColumsDataCrypto[0])) ^
            (PolinomByte(&ConstMixCrypto[(ValueOwnCrypto * 4) + 1], &ColumsDataCrypto[1])) ^
            (PolinomByte(&ConstMixCrypto[(ValueOwnCrypto * 4) + 2], &ColumsDataCrypto[2])) ^
            (PolinomByte(&ConstMixCrypto[(ValueOwnCrypto * 4) + 3], &ColumsDataCrypto[3]));
        ValueOwnCrypto++;
        if (ValueOwnCrypto == 4)
        {
            ValueOwnCrypto = 0;
...
void ENCRYPT::AddExpanseKeyCrypto(BYTE *ExKey, BYTE *Data, int i)
{
    if (i < 16)
    {
        ...
        AddExpanseKeyCrypto(ExKey, Data, ++i);
    }
    else
    {
        WordCrypto += 16;
...
void ENCRYPT::CryptRound(BYTE *ExKey, BYTE *Data)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        if (i < 9)
        {
            ByteSwappingCrypto(Data);
            ByteOffsetCrypto(Data);
            ColumnMixingCrypto(Data);
